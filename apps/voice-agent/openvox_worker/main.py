...
# Local alias for use in monkey-patches (avoids re-importing at module scope).
_logger = logger
...
class _FilterNoneChoices:
    """透传 stream，跳过 chunk.choices 为 None 的帧（Hermes usage-only 块）。

    同时累积 delta.content 到 self._text，最后在 aclose 时通过 _logger
    打 [LLM-TEXT] marker —— e2e 测试用这个拿 agent 真实回复文本。
    """
    ...
    async def __anext__(self):
        async for chunk in self._inner:
            if chunk.choices is None:
                continue
            # 累积 delta.content 用于 [LLM-TEXT] marker
            try:
            ...
            return chunk
        # 流结束：emit [LLM-TEXT]
        final = "".join(self._text_parts).strip()
        if final:
            _logger.info(f"[LLM-TEXT] {final}")
            self._text_parts.clear()
        raise StopAsyncIteration

    async def aclose(self) -> None:
        # aclose 触发时若还有未发射的文本，emit 一下（防止调用方没消费完就 close）
        final = "".join(self._text_parts).strip()
        if final:
            _logger.info(f"[LLM-TEXT] {final}")
            self._text_parts.clear()
...
# 同 STT 根因：livekit-agents 1.6.x 的子进程模型在断开时 cancel 所有 task，
# TTS 插件 _run 里 `resp.content` 的 async for 被 cancel 后 CancelledError
# 沿调用栈冒到 `_GatheringFuture`，但 gather 本身也已 cancel，导致
# inner task exception "never retrieved" → asyncio 兜底打 ERROR。
# 没有 inner recv_task 可 drain（HTTP 同步迭代器），改为在 _run 入口 swallow
# CancelledError，让上层 gather 不再收到该异常。
from livekit.plugins.volcengine.tts import SynthesizeStream as _VolcTTSSynthStream
_orig_tts_run = _VolcTTSSynthStream._run


async def _patched_tts_run(self, emitter):  # noqa: ANN001
    try:
        await _orig_tts_run(self, emitter)
    except _asyncio.CancelledError:
        # disconnect 是预期路径；不让 CancelledError 污染 worker 日志。
        return


_VolcTTSSynthStream._run = _patched_tts_run  # type: ignore[assignment]
# ───────── TTS patch 结束 ─────────
# ───────── 补丁结束 ─────────
...
class VolcengineAgent(Agent):
    ...
    async def on_enter(self) -> None:
        # 主动打招呼：原实现调 ``generate_reply(user_input="打招呼")`` 触发
        # 一次完整 LLM → TTS → 音频 链路，导致进房后 5~8 秒才能听到问候。
        # 这里换成固定的预录问候：纯 TTS，无 LLM round-trip，把首音频
        # 延迟从 ~5s 砍到 ~400ms（TTS 单次 POST 一次）。
        #
        # 用户端体感：
        #   - 进房立即听到问候（"你好呀，我是小语……"）
        #   - 用户真正开口说话时 LLM 已经在响应他的第一句
        # 这里把文字做成常量 + Unicode 短句；audio 真选什么 voice 由 .env
        # 里 volcengine.tts.voice 控制。
        logger.info("[Agent] 主动打招呼 (greeting)")
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    # Room lifecycle markers — e2e 测试用这些判断 join 真成功 / hangup 真退出。
    # "connected" 没有原生事件；用 connection_state_changed 监听进入 CONNECTED 态。

    def _on_state_changed(state) -> None:
        # ConnectionState 是 int enum: 0=DISCONNECTED, 1=CONNECTED, 2=RECONNECTING
        # 调试：记录每次状态变化
        logger.debug(f"[Worker] state changed: state={state} type={type(state).__name__}")
        if int(state) == 1:  # CONN_CONNECTED
    ...
    def _on_track_published(publication, participant) -> None:
        # publication.kind 是 int: 1=audio, 2=video
        logger.debug(f"[Track] remote published: kind={getattr(publication, 'kind', None)} by={participant.identity}")
    ...
    def _on_track_subscribed(track, publication, participant) -> None:
        logger.debug(f"[Track] remote subscribed: kind={getattr(publication, 'kind', None)} by={participant.identity}")
        if int(getattr(publication, "kind", 0)) == 1:
            logger.info(f"[Track] remote audio subscribed from {participant.identity}")

    def _on_local_track_published(publication, _track) -> None:
        logger.debug(f"[Track] local published: kind={getattr(publication, 'kind', None)}")
        if int(getattr(publication, "kind", 0)) == 1:
            logger.info(f"[Track] local mic published by {ctx.room.local_participant.identity}")

    def _on_local_track_unpublished(publication, _track) -> None:
        logger.debug(f"[Track] local unpublished: kind={getattr(publication, 'kind', None)}")
        if int(getattr(publication, "kind", 0)) == 1:
            logger.info(f"[Track] local mic unpublished by {ctx.room.local_participant.identity}")

    ctx.room.on("connection_state_changed", _on_state_changed)
    ...
    ctx.room.on("local_track_unpublished", _on_local_track_unpublished)
