...
def _repo_root() -> Path:
    # apps/voice-agent/openvox_cli.py -> parents[2] == repository root.
...
class _WorkerLauncher:
    """Launch the LiveKit worker (``main.py start``) in the foreground.

    Kept intentionally thin — it shells out to ``main.py`` so this module
    never imports the heavy ``livekit`` / ``openai`` stack. Tests inject a
    fake with the same ``start()`` surface.
    """
    ...
    def start(self) -> None:
        repo_root = _repo_root()
        main_py = repo_root / "apps" / "voice-agent" / "main.py"
        env = dict(os.environ)
        if self._config_path is not None:
            env["OPENVOX_CONFIG"] = str(self._config_path)
        result = subprocess.run(
            [sys.executable, str(main_py), "start"],
