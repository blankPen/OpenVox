...
dev 模式默认密钥是 `devkey` / `secret`，与本仓库示例 config（`livekit.api_key` / `api_secret`）一致。
...
当前唯一支持的管线是 STT → LLM → TTS 三段式（`main.py::_build_session()` 拼装，没有切换开关）。STT/TTS 走火山引擎插件，LLM 走 `openai.LLM` 直连 Hermes gateway 的 OpenAI 兼容 api_server（默认 :8642，配置在 `hermes.api_base`）。
...
| agent 启动时报 `ConfigError: missing required config key: ...` | `~/.openvox/config.json` 缺键 | 补对应业务段的字段；schema 见 `config.py` 头部注释 |
...
| `failed to connect to livekit, retrying in 0s ... WSServerHandshakeError 401` | LIVEKit cred 错了 | 核对 `~/.openvox/config.json` 里 `livekit.api_key` / `api_secret` / `url` 与 server 一致；`--dev` 模式下用 `devkey` / `secret` / `ws://localhost:7880` |
...
1. **STT 开通**：去火山控制台 `https://console.volcengine.com/voice/app` 给当前 `volcengine.stt.app_id` 勾选「流式语音识别（豆包大模型）」，否则 STT WebSocket 会 403。
2. **LiveKit Cloud / 自托管生产**：把 `~/.openvox/config.json` 里的 `livekit.{url,api_key,api_secret}` 换成真凭证或自托管 server 配置；`main.py` 不需要改。
3. **Function tools**：当前 `VolcengineAgent` 没暴露 `@function_tool`，按 [livekit-agents basic_agent 示例](https://github.com/livekit/agents/blob/main/examples/voice_agents/basic_agent.py) 加天气查询等工具即可。
4. **Dockerize**：写个 `Dockerfile` 把 `apps/voice-agent/` 打包成镜像，部署到 LiveKit Cloud Agents 或自托管 worker 池（backlog 项，当前未落地）。
