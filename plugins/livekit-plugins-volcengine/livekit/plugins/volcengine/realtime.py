...
class RealtimeSession(
    ...
    def __init__(self, realtime_model: RealtimeModel) -> None:
        ...
        # 缓冲 LLM 流式生成的文本 chunk（550 事件），在 559 事件时合并为整句打印
    ...
    async def _main_task(self) -> None:
        ...
        # 暴露给同步方法（如 generate_reply）使用，让它在 ws 连接就绪后
        # 能直接发二进制请求帧（hello_request / chat_tts_text）给服务端。
            ...
            while True:
                try:
                    ...
                    if event == 450:  # ASRInfo
                        ...
                        if is_final:
                            logger.info(f"[ASR] 📝 识别到用户说: {transcription!r}")
                            item_id = utils.shortuuid()
                            self.emit(
                        ...
                        if self._llm_text_buffer:
                            logger.info(
                                f"[Agent 回复] 🗣️ {self._llm_text_buffer!r}"
                            )
                            self._llm_text_buffer = ""
    ...
    async def update_chat_ctx(self, chat_ctx: llm.ChatContext) -> None:
        """同步 chat_ctx 到 vendor _remote_chat_ctx。

        LiveKit 框架会在每次生成前调用 update_chat_ctx，把 chat_ctx 增量同步进来。
        我们需要把 chat_ctx 里的 user messages 插入 _remote_chat_ctx，让后续
        generate_reply 能取到最后一条 user message 作为 chat_tts_text 的 content。

        实现：对比新旧 _remote_chat_ctx 长度，把 chat_ctx 中位置 >= 当前长度的
        user messages 插入。assistant messages 由 vendor 自己通过 LLMResponse
        事件插入（如果需要），这里只同步 user。
        """
        existing_ids = set(self._remote_chat_ctx._id_to_item.keys())
        new_items_added = 0
        for item in chat_ctx.items:
            ...
            if item.id in existing_ids:
                continue
            try:
                self._remote_chat_ctx.insert(None, item)
                new_items_added += 1
            except ValueError:
                pass  # 重复 ID 跳过
        if new_items_added > 0:
            logger.info(
                f"[chat_ctx] 同步了 {new_items_added} 条新 user message 到 vendor chat context"
            )
    ...
    def generate_reply(
        ...
        *,
        ...
        """触发服务端生成回复（用户说话 ASR 完成后、文本消息输入等场景）。

        实现策略：
        1. 复用 opening 路径的 _ResponseGeneration/_MessageGeneration 创建逻辑，
           立即把 generation_ev 通过 future 返回（让框架 _realtime_reply_task 拿到 event
           并启动 _realtime_generation_task 消费 message_stream）。
        2. 决定要发送给服务端的文本内容：优先 instructions，否则取 chat_ctx 中最后一条
           user message。
        3. 异步通过 chat_tts_text (opcode 500) 把文本发给服务端，触发回复生成。
           服务端响应通过现有 _recv_task 的 352/550/559/359 事件处理自动填充
           _current_item 的 channels。
        """
        fut = asyncio.Future[llm.GenerationCreatedEvent]()

        # ws 未就绪（如 main_task 还在创建 ws，或 session 已关闭）
        if getattr(self, "_ws_conn", None) is None or self._ws_conn.closed:
            fut.set_exception(
                llm.RealtimeError("RealtimeSession.generate_reply: ws not ready")
            )
            return fut

        # 决定要发送给服务端的文本
        text = instructions if utils.is_given(instructions) else ""
        if not text:
            # 从 chat_ctx 取最后一条 user message 的 content
            ctx = self.chat_ctx  # property → self._remote_chat_ctx.to_chat_ctx()
                    ...
                    if isinstance(content, str):
                        ...
                        for c in content:
                            if isinstance(c, str):
                                ...
                                # ChatContent / ContentPart 都有 text 属性
        ...
        # 创建 generation + item（与 opening 路径相同结构）
        # 清空 buffer 防止上一轮 LLM 流式 chunk 残留
        self._llm_text_buffer = ""
        self._current_generation = _ResponseGeneration(
            message_ch=utils.aio.Chan(),
            function_ch=utils.aio.Chan(),
        ...
        generation_ev = llm.GenerationCreatedEvent(
            message_stream=self._current_generation.message_ch,
            function_stream=self._current_generation.function_ch,
            user_initiated=True,  # 让 _on_generation_created 跳过
        )
        ...
        # 立即 set_result，框架 await 拿到 generation_ev 后即可开始消费 message_stream
        fut.set_result(generation_ev)

        # 异步通过 chat_tts_text (opcode 500) 发请求给服务端
        ws_conn = self._ws_conn
        loop = asyncio.get_event_loop()
        loop.create_task(self._do_chat_tts(text, ws_conn))

        return fut

    async def _do_chat_tts(
        self, text: str, ws_conn: aiohttp.ClientWebSocketResponse
    ) -> None:
        """异步通过 chat_tts_text (opcode 500) 把用户文本发给服务端。

        chat_tts_text 是 vendor 提供的文本输入协议（realtime.py:712），payload：
            {"start": True, "end": True, "content": "用户文本"}

        这是触发服务端基于 system_role + chat history + user text 生成回复的正确协议。
        """
        try:
            logger.info(f"[生成] 发送 chat_tts_text (opcode 500): {text!r}")
            await self.chat_tts_text(
                start=True, end=True, content=text, ws_conn=ws_conn
            )
            logger.info("[生成] chat_tts_text 已发送，等待服务端响应")
        except Exception:
            logger.error("[生成] chat_tts_text 发送失败", exc_info=True)
