...
- `config.py`：保留通用点路径读取，新增 provider 选择和配置写入辅助。
- `llm_provider.py`：把 provider 配置解析成不可变 `LLMSettings`，并构造 OpenAI-compatible LLM。
- `hermes_runtime.py`：Hermes CLI/API readiness、启动提示、官方 CLI 配置、备份恢复。
- `process_runtime.py`：拥有关系明确的子进程启动、健康等待、pid/state 文件和优雅停止。
- `openvox_cli.py`：`init/start/stop/status/doctor/hermes setup` 的 argparse 入口。
- `main.py`：只把现有 LLM 构造切换到 `llm_provider.build_llm`，不改 LiveKit 会话逻辑。
- `tests/`：每个 runtime 单元对应独立 pytest 文件，并保留现有 LiveKit 回归测试。
- `pyproject.toml`：声明 `openvox` console script。
- `scripts/openvox`：未安装包时的仓库内 CLI wrapper。
- `Taskfile.yaml`、`scripts/start.sh`、`README.md`、`CLAUDE.md`：统一命令和运行说明。
...
- Modify: `config.py:0-106`
- Modify: `main.py:281-301`
- Modify: `tests/test_main_build_session.py:20-68`
...
**Files:**
- Create: `hermes_runtime.py`
- Create: `tests/test_hermes_runtime.py`
- Modify: `config.py`（配置写入辅助）
...
```bash
git add hermes_runtime.py tests/test_hermes_runtime.py config.py
...
**Files:**
- Create: `process_runtime.py`
- Create: `agentd_runtime.py`
- Create: `tests/test_process_runtime.py`
- Create: `tests/test_agentd_runtime.py`
...
```bash
git add process_runtime.py agentd_runtime.py tests
...
**Files:**
- Create: `openvox_cli.py`
- Create: `tests/test_openvox_cli.py`
- Create: `scripts/openvox`
- Modify: `pyproject.toml:1-18`
...
```bash
git add openvox_cli.py tests/test_openvox_cli.py scripts/openvox pyproject.toml
...
**Files:**
- Modify: `scripts/start.sh:1-124`
- Modify: `Taskfile.yaml:26-46,49-52`
- Modify: `README.md`
- Modify: `CLAUDE.md`
- Create: `tests/test_start_script_contract.py`
...
```bash
git add scripts/start.sh Taskfile.yaml README.md CLAUDE.md tests/test_start_script_contract.py
...
**Files:**
- Modify: `tests/test_main_build_session.py`
- Modify: `tests/test_openai_llm_hermes_compat.py`
- Create: `tests/test_agentd_smoke.py`
- Modify: `README.md`（验证命令）
...
OPENVOX_CONFIG="$HOME/.openvox/config.json" \
  python openvox_cli.py doctor hermes
```
...
```bash
OPENVOX_AGENTD_E2E=1 python openvox_cli.py start --yes
OPENVOX_AGENTD_E2E=1 pytest tests/test_agentd_smoke.py -q
python openvox_cli.py status --json
python openvox_cli.py stop
```
