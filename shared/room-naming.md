...
> **历史**：v0.3.0 之前 agent_name 是 `openz`（外部 LiveKit 派单表的旧名）。v0.3.0 起统一为 `openvox`。老 config 里的 `livekit.agent_name` 字段需手动改 / 重跑 `openvox init`，否则 dispatch 会失败。
...
后端 worker 启动后默认注册到 `agent_name = "openvox"`（来自 config）。派单方式：
