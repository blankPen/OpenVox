...
def test_start_script_delegates_to_openvox_cli():
    ...
    assert "openvox_cli.py" in text, (
        ...
        "(see openvox_cli.py); re-implementing provider logic in shell is "
...
def test_start_script_avoids_port_based_kill():
    ...
    # that ``openvox_cli.py`` owns the supervised ``agentd`` lifecycle.
    forbidden_tokens = (
        "lsof -ti",
        "xargs kill -9",
        "WORKER_PORT",
...
def test_start_script_preserves_legacy_subcommands():
    ...
    # for downstream scripts, but each branch should defer to openvox_cli.py
    ...
    assert text.count("openvox_cli.py") >= 1
...
def test_start_script_does_not_require_live_livekit_process():
    ...
    ``openvox_cli.py status`` / ``openvox_cli.py start`` exclusively.
    """
    text = _read_script()
    assert "lsof" not in text, (
        "scripts/start.sh still shells out to lsof; it should delegate all "
        "process / port inspection to openvox_cli.py status."
    )
